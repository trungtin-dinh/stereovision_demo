...
def read_markdown_file(path: str) -> str:
    ...
    if not file_path.exists():
        return f"## Missing file\n\nThe file `{path}` was not found next to `app_sl.py`."
...
def validate_positive_float(value, name: str) -> float:
    ...
    if not np.isfinite(value) or value <= 0:
        raise ValueError(f"{name} must be strictly greater than 0.")
...
def validate_disparity_parameters(
    ...
    if max_num_disparities < 16:
        raise ValueError("The processed image width is too small for StereoSGBM.")

    num_disparities = int(num_disparities)
    if num_disparities < 16:
        raise ValueError("Number of Disparities must be at least 16.")
    ...
    if num_disparities % 16 != 0:
        raise ValueError("Number of Disparities must be a multiple of 16.")

    max_block_size = min(image_width, image_height)
    block_size = int(block_size)
    if block_size < 3:
        raise ValueError("Block Size must be at least 3.")
...
def run_stereo_demo(
    ...
    if left_image is None or right_image is None:
        raise ValueError("Please upload exactly two images or use the default example pair.")
    ...
    if valid_disparity.size == 0:
        raise ValueError("Disparity could not be computed correctly for this image pair.")
    ...
    return {
        "summary": summary,
        "point_cloud_figure": point_cloud_figure,
        "depth_view": depth_view,
        "disparity_view": disparity_view,
        "matches_view": matches_view,
        "used_left_rgb": used_left_rgb,
        "used_right_rgb": used_right_rgb,
        "ply_path": ply_path,
    }
...
def initialize_state() -> None:
    defaults = {
        "results": None,
        "doc_fr_title": DOC_FR_TITLES[0] if DOC_FR_TITLES else "Documentation",
        "doc_en_title": DOC_EN_TITLES[0] if DOC_EN_TITLES else "Documentation",
    }
...
- [ETH3D Stereo Benchmark](https://www.eth3d.net/)
    ...
    use_default_pair = st.checkbox("Use Default Middlebury Pair", value=True)

    left_column, right_column = st.columns(2)
    with left_column:
        left_uploaded = st.file_uploader("Left Image", type=["png", "jpg", "jpeg", "bmp", "tif", "tiff"], key="left_image")
    with right_column:
        right_uploaded = st.file_uploader("Right Image", type=["png", "jpg", "jpeg", "bmp", "tif", "tiff"], key="right_image")

    left_image = None
    right_image = None

    if use_default_pair and left_uploaded is None and right_uploaded is None:
        try:
            left_image = load_image_from_url(DEFAULT_LEFT_URL)
            right_image = load_image_from_url(DEFAULT_RIGHT_URL)
        except Exception as exc:
            st.warning(f"Default images could not be loaded: {exc}")
    else:
        left_image = load_uploaded_image(left_uploaded)
        right_image = load_uploaded_image(right_uploaded)

    if left_image is not None and right_image is not None:
        preview_left, preview_right = st.columns(2)
        with preview_left:
            st.image(left_image, caption="Left Image", use_container_width=True)
        with preview_right:
    ...
    with st.expander("Settings", expanded=False):
        ...
        with row_1_col_2:
            baseline_m = st.number_input("Baseline (Meters)", min_value=0.000001, value=0.10, step=0.01, format="%.6f")
        with row_1_col_3:
            auto_resize = st.checkbox("Automatically Resize For Speed", value=True)

        rectification_mode = st.radio(
            "Rectification Mode",
            options=["Auto", "Already Rectified", "Estimate And Rectify"],
            index=0,
            horizontal=True,
        )
        ...
        with row_2_col_1:
            auto_rectification_threshold_px = st.number_input(
                "Auto Rectification Threshold (Pixels)",
                min_value=0.000001,
                value=2.0,
                step=0.5,
                format="%.2f",
        ...
        with row_2_col_2:
            point_count = st.number_input(
                "Point Count",
                min_value=100,
                max_value=300000,
                value=DEFAULT_POINT_COUNT,
                step=1000,
            )

        row_3_col_1, row_3_col_2 = st.columns(2)
        with row_3_col_1:
            num_disparities = st.number_input(
        ...
        row_4_col_1, row_4_col_2, row_4_col_3 = st.columns(3)
        with row_4_col_1:
            uniqueness_ratio = st.number_input(
                "Uniqueness Ratio",
                min_value=0,
                max_value=100,
                value=5,
        ...
        with row_4_col_2:
            speckle_window_size = st.number_input(
                "Speckle Window Size",
                min_value=0,
                max_value=500,
                value=100,
        ...
        try:
            with st.spinner("Running stereo reconstruction..."):
                st.session_state.results = run_stereo_demo(
                    ...
                    auto_rectification_threshold_px=auto_rectification_threshold_px,
                    num_disparities=num_disparities,
                    block_size=block_size,
                    uniqueness_ratio=uniqueness_ratio,
                    speckle_window_size=speckle_window_size,
    ...
    if results is None:
        st.info("Run the reconstruction to display the point cloud, disparity map, depth map, feature matches, rectified images, and PLY export.")
        return
    ...
    with output_tabs[0]:
        st.plotly_chart(results["point_cloud_figure"], use_container_width=True)

    with output_tabs[1]:
        depth_col, disparity_col = st.columns(2)
        with depth_col:
            st.image(results["depth_view"], caption="Depth Map", use_container_width=True)
        with disparity_col:
            st.image(results["disparity_view"], caption="Disparity Map", use_container_width=True)

    with output_tabs[2]:
        st.image(results["matches_view"], caption="ORB Matches and RANSAC Inliers", use_container_width=True)

    with output_tabs[3]:
        rect_left_col, rect_right_col = st.columns(2)
        with rect_left_col:
            st.image(results["used_left_rgb"], caption="Left Image", use_container_width=True)
        with rect_right_col:
            st.image(results["used_right_rgb"], caption="Right Image", use_container_width=True)
    ...
    with output_tabs[4]:
        ...
        if ply_path is not None and Path(ply_path).exists():
            ...
            st.download_button(
                ...
                data=ply_bytes,
                file_name="stereo_point_cloud.ply",
...
def render_documentation_tab(language: str) -> None:
    if language == "fr":
        ...
        sections = DOC_FR_SECTIONS
        state_key = "doc_fr_title"
    else:
        titles = DOC_EN_TITLES
...
def main() -> None:
    ...
    with tabs[1]:
        render_documentation_tab("fr")

    with tabs[2]:
        render_documentation_tab("en")
